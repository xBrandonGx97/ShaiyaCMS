...
class WhEdit
{
    ...
    public function getItemInfo()
    {
        ...
        FROM ' . table('shUserWh') . ' c INNER JOIN ' . table('shItems') . ' i on i.ItemID = c.ItemID
    ...
    {
        try {
            $update = DB::table(table('shUserWh'))
            ...
            $this->logSys->createLog('Edited item: ' . $this->getItemName() . ' of user: ' . $this->userId);
            return 'Edited item: ' . $this->getItemName();
        } catch (\Illuminate\Database\QueryException $e) {
            $this->logSys->createLog('Failed to edit item: ' . $this->getItemName() . ' of user: ' . $this->userId);

            ...
            if (isset($_COOKIE['Vote4DP']) && isset($_COOKIE['VoteID'])) {
                ...
                if ($rowCount > 0) {
                    foreach ($res as $data) {
                        ...
                        // site 2
                        elseif ($data['VoteSite'] == 'nr2' && $this->referer == $this->referer2) {
                            $this->MSSQL->query('UPDATE PS_UserData.dbo.Users_Master
                                    SET Point = Point+:point
                                    WHERE UserID = :user');
                            $this->MSSQL->bind(':point', $Point);
                        ...
                        // site 3
                        elseif ($data['VoteSite'] == 'nr3' && $this->referer == $this->referer3) {
                            $this->MSSQL->query('UPDATE PS_UserData.dbo.Users_Master
                                    SET Point = Point+:point
                                    WHERE UserID = :user');
                            $this->MSSQL->bind(':point', $Point);
                        ...
                        // site 4
                        elseif ($data['VoteSite'] == 'nr4' && $this->referer == $this->referer4) {
                            $this->MSSQL->query('UPDATE PS_UserData.dbo.Users_Master
                                    SET Point = Point+:point
                                    WHERE UserID = :user');
                            $this->MSSQL->bind(':point', $Point);

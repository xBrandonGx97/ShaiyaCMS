        ...
        public function PlayersOnline()
        {
            $select = (
                '
                    ...
                    (SELECT COUNT(*) FROM ' . $this->MSSQL->getTable('SH_CHARDATA') . ' WHERE LoginStatus=1 AND Faction = 0)
                    AS AoL,
                    (SELECT COUNT(*) FROM ' . $this->MSSQL->getTable('SH_CHARDATA') . ' WHERE LoginStatus=1 AND Faction = 1)
                    AS UoF
                '
            );

            //  make this easier in the query builder

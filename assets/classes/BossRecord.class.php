		...
		function get_Record(){
			echo '<div class="table-responsive">';
				echo '<table class="table table-dark table-striped tac">';
					...
					foreach($this->MobID as $value){
						$this->sql = ("
										SELECT TOP 1 [MobName],[CharName],[ActionTime]
										FROM ".$this->db->get_TABLE("LOG_BOSS_DEATH")."
										WHERE MobID=?
										ORDER BY ActionTime DESC
						");
						$this->query=$this->db->conn->prepare($this->sql);
						$this->query->bindValue(1, $value, PDO::PARAM_INT);
						$this->query->execute();
						$this->fet = $this->query->FETCH();

						$this->MobName		=	$this->fet["MobName"];
						$this->CharName		=	$this->fet["CharName"];
						$this->ActionTime	=	$this->fet["ActionTime"];

	...
	class Nav{
		...
		function NAV_TOP($Zone){
				...
				echo '<nav class="navbar navbar-expand-lg navbar-dark fixed-top bg-dark">';
					...
					echo '<div class="collapse navbar-collapse" id="navbarsExample05">';
						echo '<ul class="navbar-nav mr-auto">';
							...
							$this->ds_nav_dropdown('Server Information','About','ABOUT','Boss Records','BOSS_RECORDS','Download','DOWNLOAD','DropFinder','DROPFINDER','Guild Rankings','GUILD_RANK','News','NEWS','Patch Notes','PATCH','Terms of Service','TERMS',false,false,1,0);
							/* Dropdown Func
						...
						if(!$this->User->LoggedIn() || !isset($_SESSION['Status'])){
							...
							echo '</li>';
							echo '<li class="nav-item">';
								echo '<a class="nav-link" href="?'.$this->Setting->PAGE_PREFIX.'=LOGIN">Login</a>';
							echo '</li>';
						}
						else{
							$profileSQL = ("
							...
							$queryProfile->bindValue(1, $_SESSION["uuid"], PDO::PARAM_INT);
							$queryProfile->execute();
							$playerprofile = $queryProfile->FETCH(PDO::FETCH_NUM);
								try{
									echo '<a class="nav-link" href="#"><img class="img-fluid" src="assets/Themes/Standard/images/profile/avatars/'.$playerprofile[18].'" width="30" height="30"> '.$this->User->UserLoginStatus.'</a>';
			...
			echo '<nav class="navbar navbar-expand navbar-dark bg-dark acp-bg fixed-top">';
					echo '<a class="navbar-brand mr-1" href="?pageid=DASHBOARD">
		...
		function NAV_SIDE($Zone){
			...
			echo '<ul class="sidebar navbar-nav">';
				echo '<li class="nav-item active">';
					echo '<a class="nav-link" href="?pageid=DASHBOARD">';

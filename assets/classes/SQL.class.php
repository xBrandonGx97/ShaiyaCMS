<?php
	class SQL{
		...
		function __construct($Data,$db,$Setting,$Template,$User/*,$Messenger*/){
			$this->db			=	$db;
			$this->Data			=	$Data;
...
#			$this->Messenger	=	$Messenger;
		...
		function REGISTER_USER_GAME($UserID,$Password,$UserIP){
			$ret	=	false;
		...
		function REGISTER_USER_WEB($UserID,$Password,$DisplayName,$DOB,$Gender,$Referer,$SecQuestion,$SecAnswer,$ActivationKey,$Email,$UserIP,$Avatar){
			$ret	=	false;
			...
			');
			$stmt = $this->db->conn->prepare($sql);
			$params = array($UserID,$Password,$DisplayName,$DOB,$Gender,$Referer,$SecQuestion,$SecAnswer,$ActivationKey,$Email,$UserIP,$Avatar);
			$stmt->execute($params);
	...
	function _get_serverStatus($IP,$LoginPort,$GamePort){
		$ServerIP	=	$IP;
		$ServerPorts = array($LoginPort, $GamePort);+
		$LoginConn = @fsockopen($ServerIP, $ServerPorts[0], $errno, $errstr, 2);
		$GameConn = @fsockopen($ServerIP, $ServerPorts[1], $errno, $errstr, 2);
		if ($LoginConn){

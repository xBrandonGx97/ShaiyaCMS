<?php
	...
	#	Create DATABASE LOG
	$this->LogSys->createLog("Viewed Account IP Search");
	
	$id = isset($_POST["CharName"]) ? $this->Data->escData(trim($_POST["CharName"])) : false;
	$CharID = isset($_POST["CharID"]) ? $this->Data->escData(trim($_POST["CharID"])) : false;
	
	if(isset($_POST['SC'])){
		# Start Template
		$this->Tpl->_do_ACP_pageHeader("","",false,"12","<i class='fas fa-table'></i> Select an account to get All characters Associated to it.");
		if(empty($id)){
		...
		$stmt->execute($args);
			...
			while($res1=$stmt->fetch()){
				...
				$sql1 = ("
							SELECT * FROM ".$this->db->get_TABLE("SH_USERDATA")." WHERE UserIp=?
				");
	...
	elseif (isset($_POST['SCI'])){
		# Start Template
		$this->Tpl->_do_ACP_pageHeader("","",true,"6","Results");
		...
		$sql = ("
					SELECT * FROM ".$this->db->get_TABLE("SH_CHARDATA")." WHERE UserUID=? AND Del=? ORDER BY Slot
		");
		$stmt=$this->db->conn->prepare($sql);
		$args = array($CharID,0);
		$stmt->execute($args);
	...
	# Start Template
	$this->Tpl->_do_ACP_pageHeader("","",true,"6","Search All Accounts Associated to IP of a Character");
		echo '<form method="POST">';

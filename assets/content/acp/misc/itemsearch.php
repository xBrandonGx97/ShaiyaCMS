<?php
	...
	#	Create DATABASE LOG
	$this->LogSys->createLog("Viewed Item Search Log");
	
	# Form Data
	$count=0;
	# Start Template
	$this->Tpl->_do_ACP_pageHeader("","",true,"6","Item Search By Category");

	if(isset($_POST['submit'])){

	$itemid = isset($_POST["ItemID"]) ? $this->Data->escData(trim($_POST["ItemID"])) : false;
	$sql = ("
				SELECT * FROM ".$this->db->get_TABLE("SH_ITEMS")." WHERE Type=? AND LEFT(ItemName, 1) != '?' ORDER BY ItemID
	");

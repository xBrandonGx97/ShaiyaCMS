<?php
    ...
    $player		=	isset($_POST["player"])		?	$this->Data->escData(trim($_POST["player"]))	:	false;
	
	# Query    
    $sql = ("
                SELECT [CD].[CharID], [CD].[ChatType], [CD].[TargetName], [CD].[ChatData], [CD].[ChatTime], [CD].[MapID],[C].[CharName],[C].[Family]
                FROM ".$this->db->get_TABLE("SH_CHATLOG")." AS [CD]
                INNER JOIN ".$this->db->get_TABLE("SH_CHARDATA")." AS [C] ON [CD].[CharID] = [C].[CharID]
	...
	#	Create DATABASE LOG
	$this->LogSys->createLog("Viewed Player Chat Search");
    ...
    if (isset($_POST['submit'])) 
    {
        ...
        $this->Tpl->_do_ACP_pageHeader("","",false,"12","Player Chat Search");
        ...
        $this->Tpl->_do_ACP_pageHeader("","",true   ,"6","Player Chat Search");

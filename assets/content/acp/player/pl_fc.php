...
#	$sql = "SELECT UserID,Pw FROM  PS_UserData.dbo.Users_Master WHERE UserID = ?";
#	$params = array($userid);
#	$useruid = sqlsrv_query( $conn, $sql, $params, $options );
    
        $sql	=	("
						SELECT UserID,Pw FROM  PS_UserData.dbo.Users_Master WHERE UserID = ?"
		  );
...
#			$sql = "SELECT UserID,Pw FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?";
#			$params = array($userid,$pass);
#			$useruid1 = sqlsrv_query( $conn, $sql, $params, $options );
        
         $sql1	=	("
						SELECT UserID,Pw FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?"
		  );
		$stmt1	=	odbc_prepare($cxn1,$sql1);
		$args1	=	array($userid,$pass);
...
#		$sql = "SELECT * FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?";
#		$params = array($userid,$pass);
#		$useruid2 = sqlsrv_query( $conn, $sql, $params, $options );
        
        $sql2	=	("
						SELECT * FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?"
		  );
		$stmt2	=	odbc_prepare($cxn2,$sql2);
		$args2	=	array($userid,$pass);
		$prep2	=	odbc_execute($stmt2,$args2);
		
		echo " Account and Pass match </br>";
	}

	while ($row = sqlsrv_fetch_array($stmt2))
	{
		echo " Points = " . $row['Point'] . "</br>";
		if ($row['Point'] < $point) {
				echo " Points = " . $row['Point'] . "</br>";
				die('Account ' . $userid . ' dosent have enough DP');
		} 
		else
		/*{
				//create_log
				//$INSERT = mssql_query('SELECT * FROM  GM_Stuff.dbo.Login WHERE IP = \'' . $ip . '\'');
				$sql = "SELECT * FROM  GM_Stuff.dbo.Login WHERE IP = ?";
				$params = array($ip); 
				$INSERT = sqlsrv_query( $conn, $sql, $params, $options );

				if (sqlsrv_num_rows($INSERT) == 0) {
					die('Admin Account Log returned no results');
				}
				else
				{
					while ($Log = sqlsrv_fetch_array($INSERT, SQLSRV_FETCH_ASSOC)) //while ($Log = mssql_fetch_assoc($INSERT))
					{
						//mssql_query('INSERT INTO GM_Stuff.dbo.Log (StaffID,StaffIP,Account,Actiondef,Date) VALUES 
						//(\''.$Log['UserID'].'\',\''.$ip.'\',\'' . $userid . '\',\' Faction Change \',GETDATE())');	
						$sql = "INSERT INTO GM_Stuff.dbo.Log (StaffID,StaffIP,Account,Actiondef,Date) VALUES (?,?,?,?,?)";
						$params = array($Log['UserID'],$ip,$id,$userid . " Faction Change  ", $date);
...
#	$sql = "SELECT * FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?";
#	$params = array($userid, $pass);
#	$useruid3 = sqlsrv_query( $conn, $sql, $params, $options );
    
    $sql3	=	("
						SELECT * FROM  PS_UserData.dbo.Users_Master WHERE UserID = ? and PW = ?"
    );
    $stmt3	=	odbc_prepare($cxn2,$sql3);
    $args3	=	array($userid,$pass);
    $prep3	=	odbc_execute($stmt3,$args3);

	if (odbc_num_rows($stmt3)>0){
		die('Account and Password mixmatch');
	}
	else
	{
		while ($row = odbc_fetch_array($useruid3))  
		{
				//$query1 = 'DELETE FROM PS_GameData.dbo.CharSkills WHERE CharID in (SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID=' . $row['UserUID'] . ' AND [Level]>1 AND Del=0 AND Grow>0) AND SkillID<>4';
#				$sql = "SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID= ? AND [Level]>1 AND Del=0 AND Grow>0";
#				$params = array($row['UserUID']);
#				$med = sqlsrv_query( $conn, $sql, $params, $options);
            
                $sql4	=	("
						SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID= ? AND [Level]>1 AND Del=0 AND Grow>0"
                );
                $stmt4	=	odbc_prepare($cxn2,$sql4);
                $args4	=	array($row['UserUID']);
                $prep4	=	odbc_execute($stmt4,$args4);
            
				while ($stuff = odbc_fetch_array($stmt4))  
				{
					$row1_charId = $stuff['CharID'];
				}
				echo " charID = " . $row1_charId . "</br>";
				
#				$sql = "SELECT * FROM PS_GameData.dbo.CharSkills WHERE CharID = ?"; // AND SkillPoint<>4";//SkillID<>4";
#				$params = array($row1_charId);
#				$query1 = sqlsrv_query( $conn, $sql, $params, $options);
            
                $sql5	=	("
						SELECT * FROM PS_GameData.dbo.CharSkills WHERE CharID = ?"
                );
                $stmt5	=	odbc_prepare($cxn2,$sql5);
                $args5	=	array($row1_charId);
                $prep5	=	odbc_execute($stmt5,$args5);
            
				if (!sqlsrv_query( $conn, $sql, $params, $options)) { //if (!mssql_query($query1)) {
					die("Account $userid Skill Deletion Unsuccessfull!");
				}
				else
				{
						// $charid = @mssql_query('SELECT * FROM  PS_GameData.dbo.Chars WHERE UserID = \'' . $userid . '\'');
						$sql = "SELECT * FROM  PS_GameData.dbo.Chars WHERE UserID = ?";
						$params = array($userid);
						$charid = sqlsrv_query( $conn, $sql, $params, $options );
				}
		}
	}
		
	if (odbc_num_rows($charid)>0) {
		die('Account and Password mixmatch');
	}
	else
	{	
		while ($row1 = odbc_fetch_array($charid)) 
		{
				$row1_level = $row1['Level'];
				$row1_id = $row1['UserUID'];
		}
	} 
		echo " UserUID = " . $row1_id . "</br>";
		echo " Level = " . $row1_level . "</br>";

					die('STOP! This script is incomplete! Talk to Tyler.');
					$query2 = 'UPDATE PS_GameData.dbo.Chars SET SkillPoint = (' . $row1['Level'] . '-1)*(CASE WHEN Grow = 1 THEN ' . $NM . ' WHEN Grow = 2 THEN ' . $HM . ' WHEN Grow = 3 THEN ' . $UM . '  END) WHERE [Level]>1 AND Del=0 AND Grow>0 AND UserID=\'' . $userid . '\'';
					if (!mssql_query($query2)) {
						die("account $userid Skill Reset Unsuccessfull!");
					}

					$query3 = 'DELETE FROM PS_GameData.dbo.CharQuests WHERE CharID in (SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserID=\'' . $userid . '\')';					
					if (!mssql_query($query3)) {
						die("account $userid Quest Deleation Unsuccessfull!");
					}

					$query4 = 'DELETE FROM PS_GameData.dbo.GuildChars WHERE CharID in (SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID=' . $row1['UserUID'] . ')';					
					if (!mssql_query($query4)) {
						die("account $userid Guild Drop Unsuccessfull!");
					}
				
					$query5 = 'DELETE FROM PS_GameData.dbo.FriendChars WHERE CharID in (SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID=' . $row1['UserUID'] . ')';
					if (!mssql_query($query5)) {
							die("account $userid Friended Chars Deleation Unsuccessfull!");
					}
						
					$query6 = 'DELETE FROM PS_GameData.dbo.FriendChars WHERE FriendID in (SELECT CharID FROM PS_GameData.dbo.Chars WHERE UserUID=' . $row1['UserUID'] . ')';
					...
					else
					{
							...
							else
							{
								while ($row3 = mssql_fetch_array($row2)) {
									$query7 = 'UPDATE PS_GameData.dbo.UserMaxGrow SET Country=(CASE WHEN Country = 0 THEN 1 WHEN Country = 1 THEN 0 END) WHERE UserUID=' . $row3['UserUID'] . '';
								}
							}
					}
					...
					$query8 = 'UPDATE PS_GameData.dbo.Chars SET [Family] = (CASE WHEN Family=0 AND Job=5 THEN 2 WHEN Family=0 AND Job IN (0,1) THEN 3 
...
WHERE UserUID=' . $row3['UserUID'] . '';
					...
					$query9 = 'UPDATE PS_GameData.dbo.Chars SET Map=42, PosX=63,PosY=2, PosZ=57 WHERE UserID=\'' . $userid . '\'';
				        ...
				        echo '<div class="panel panel-headline">';
					       ...
					       echo '</div>';
					           echo '<div class="panel-body">';
                                echo '<p>Faction Change</p>';
                                echo '<form class="form-inline" method="POST">';
                                echo '<div class="form-group">';
                                echo '<input type="text" name="userid" placeholder="User ID" class="form-control tac b_i"/>';
                                echo '<input type="password"name="pass" placeholder="Password" class="form-control tac b_i"/>';

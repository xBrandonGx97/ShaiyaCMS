<?php
	...
	# Start Template
	$this->Tpl->_do_ACP_pageHeader("","",true,"6","Resurrect A Character");
    
	$login = isset($_POST["login"]) ? $this->Data->escData(trim($_POST["login"])) : false;
	$class = array(
		0 => 'Warrior', 1 => 'Guardian', 2 => 'Assasin', 3 => 'Hunter', 4 => 'Pagan', 5 => 'Oracle',
		6 => 'Fighter', 7 => 'Defender', 8 => 'Ranger', 9 => 'Archer', 10 => 'Mage', 11 => 'Priest'
	);
	$toon = isset($_POST["char"]) ? $this->Data->escData(trim($_POST["char"])) : false;
	...
	if (isset($_POST['submit'])) {
		...
		$sql = ("
					SELECT * FROM ".$this->db->get_TABLE("SH_USERDATA")." WHERE UserID=?
		");
		...
		if($rowCount==0){
			...
			$sql2 = ("
						SELECT umg.Country, c.Family, c.CharName, c.CharID, c.Job, c.Level FROM ".$this->db->get_TABLE("SH_UMG")." AS umg 
						INNER JOIN ".$this->db->get_TABLE("SH_CHARDATA")." AS c ON 
		...
		$login = isset($_POST["username"]) ? $this->Data->escData(trim($_POST["username"])) : false;
		$slot  = -1;
		$sql1 = ("
					...
					LEFT JOIN
					(SELECT c.Slot
					FROM ".$this->db->get_TABLE("SH_USERDATA")." AS um
					INNER JOIN ".$this->db->get_TABLE("SH_CHARDATA")." AS c ON c.UserUID = um.UserUID
					WHERE um.UserID=?
        ...
        if ($slot['OpenSlot'] > -1 && $slot['OpenSlot'] < 5) {
			$sql = ("
						UPDATE ".$this->db->get_TABLE("SH_CHARDATA")."
			...
			$this->LogSys->createLog("Resurrected Character: ".$toon2[0]."");
		} else {

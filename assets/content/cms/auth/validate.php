...
*/
	...
	if(isset($_POST["sub_login"])){
		...
		if(count($errors_body)==0){
			$sql = ("
						SELECT * FROM ".$this->db->get_TABLE("SH_USERDATA")." WHERE UserID=? AND PwPlain=?
			");
	...
	elseif(isset($_POST["sub_reg"])){
		...
		if(count($errors_body)==0){
			...
			$REG_WEB	=	$this->SQL->REGISTER_USER_WEB($UserID,$Password,$DisplayName,$DOB,$Gender,$Referer,$SecQuestion,$SecAnswer,$ActivationKey,$Email,$UserIP,$Avatar);
		}
		if(count($errors_body)){
			echo '<ul>';
			foreach($errors_body as $error){

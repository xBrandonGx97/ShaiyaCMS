<?php
	...
	#	Create DATABASE LOG
	LogSys::createLog("Viewed Account Edit");

	$acc	=	isset($_POST["acc"]) ? Settings::$purifier->purify(trim($_POST["acc"])) : false;
	$char	=	false;
	...
	if(isset($_POST['submit']) || strlen($char)>1){
		...
		$tsql = ("
					...
					WHERE UserID=?
		");
		$getAccount = Database::connect()->prepare($tsql);
		...
		if($UserCount>0){
			...
			Template::doACP_Head("","",false,"12","Current Status of ".$acc."");
			echo '<p class="f_18 u">';
	...
	elseif(isset($_POST['submit2'])){
		...
		echo '<a href="./acp-acc-edit" class="btn btn-sm btn-primary m_auto" style="margin-top:15px">Go Back</a>';
	}
	else{
		# Start Template
		Template::doACP_Head("","",true,"6","Edit Account");
		echo '<form method="POST">';

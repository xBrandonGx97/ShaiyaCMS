<?php
	...
	$GuildID = isset($_POST["GuildID"]) ? Settings::$purifier->purify(trim($_POST["GuildID"])) : false;
	$CharName = isset($_POST["CharName"]) ? Settings::$purifier->purify(trim($_POST["CharName"])) : false;

	if (isset($_POST['SCN'])){
    # Start Template
	Template::doACP_Head("","",true,"6");

	$sql6 = ("
				SELECT * FROM ".Database::getTable("SH_GUILDS")." WHERE GuildID=?
	");
...
elseif (isset($_POST['CNE']))
{
    ...
    while($Row=$stmt6->fetch()){
        LogSys::createLog("Deleted Guild: ".$Row['GuildName']."");
    }

	$sql = ("
				DELETE FROM ".Database::getTable("SH_GUILDS")." WHERE GuildID=?
	");
	...
	$sql1 = ("
				DELETE FROM ".Database::getTable("SH_GUILD_CHARS")." WHERE GuildID=?
	");
...
elseif (isset($_POST['SC'])){
	...
	$sql4 = ("
				SELECT GuildName,GuildID FROM ".Database::getTable("SH_GUILDS")." WHERE GuildName=?
	");
...
elseif (isset($_POST['SCI'])){
	...
	# Start Template
	Template::doACP_Head("","",true,"6","Disband Guild");
	echo '<form method="POST">';
		echo '<div class="form-group mx-sm-3 mb-2">';

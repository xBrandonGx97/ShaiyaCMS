<?php
	...
	if(isset($_POST['SCN'])){
		...
		$sql = ("
					...
					FROM ".Database::getTable("SH_CHARITEMS")." c
		...
		try{
			Template::doACP_Head("","",false,"12","Edit Player Items");
	...
	elseif(isset($_POST['CNE'])){
		...
		$sql = ("
					UPDATE ".Database::getTable("SH_CHARITEMS")."
	...
	elseif(isset($_POST['SC'])){
		...
		try{
			if($rowCount>0){
				Template::doACP_Head("","",true,"6","Edit Player Items");
					echo '<form method="POST">';
						echo '<div class="table-responsive">';
							echo '<table class="table table-dark">';
								echo '<thead>';
	...
	elseif(isset($_POST['SCI'])){
		...
		$sql = ("
					...
					FROM ".Database::getTable("SH_CHARITEMS")." AS CI
					INNER JOIN ".Database::getTable("SH_ITEMS")." AS I ON I.ItemID=CI.ItemID
					WHERE CI.CharID=?
					ORDER BY CI.Bag
		");
		$stmt=Database::connect()->prepare($sql);

<?php
	...
	#	Create DATABASE LOG
	LogSys::createLog("Viewed Homepage Editor");
	...
	if(isset($_POST['submit'])){
		...
		if(empty($errors)){
            $sql = ("
							INSERT INTO ".Database::getTable("HOMEPAGE")."
							(title,detail)
							VALUES(?,?)
	        ");
	...
	if(isset($_POST['deleteBtn'])){
		...
		foreach($deletes as $delete){
            $sql = ("
							DELETE FROM ".Database::getTable("HOMEPAGE")."
							WHERE RowID=?
	        ");
            $stmt = Database::connect()->prepare($sql);
			$params = array($delete);
			...
			$deleteSuccess = '<strong>SUCCESS:</strong> News Has Been Removed!';
								...
								# Start Template
								Template::doACP_Head("","",false,"12","Homepage Content Control");
			                         echo '<div id="tabs">';
				                        echo '<ul>';
                                            ...
                                            echo '<div id="tabs-2">';
                                                ...
                                                echo '<form class="form-inline" method="POST">';
                                                    echo '<table cellpadding="0" cellspacing="0" width="100%" class="table table-dark">';
                                                        ...
                                                        echo '</tr>';
                                                        $sql = ("
                                                                        SELECT * FROM ".Database::getTable("HOMEPAGE")." ORDER BY RowID ASC
	                                                    ");
                                                        $stmt = Database::connect()->prepare($sql);
                                                        $stmt->execute();
                                                    while($show=$stmt->fetch(PDO::FETCH_NUM)){
                                                        echo '<tr>';
                                                            echo '<th><input type="checkbox" name="deleteCheck[]" value="'.$show[0].'"/></th>';
                                                            echo '<td>'.Settings::$purifier->purify(htmlentities($show[0])).'</td>';
                                                            echo '<td>'.Settings::$purifier->purify(htmlentities($show[1])).'</td>';
                                                            echo '<td>'.Settings::$purifier->purify(htmlspecialchars_decode(htmlentities($show[2]))).'</td>';
                                                            echo '<td>'.Settings::$purifier->purify(htmlentities(date("m/d/y h:i A", strtotime($show[3])))).'</td>';
                                                        echo '</tr>';
                                                    }
                                                    echo '</table>';
                                                    echo '<br />';

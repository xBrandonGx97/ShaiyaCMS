<?php
	...
	$path = 'assets/Themes/shCMS/images/profile/headers/';
	...
	$path1 = 'assets/Themes/shCMS/images/profile/avatars/';
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST'){
		if(!isset($_FILES['userFile1']) || $_FILES['userFile1']['error'] == UPLOAD_ERR_NO_FILE) {
			...
			$sql = ("
						UPDATE ".Database::getTable('WEB_PRESENCE')." SET Avatar='{$ImageName1}' Where UserUID=?
			");
...
#			$success = "Updated Avatar!";
		...
		if(!isset($_FILES['userFile']) || $_FILES['userFile']['error'] == UPLOAD_ERR_NO_FILE) {
			...
			$sql = ("
						UPDATE ".Database::getTable('WEB_PRESENCE')." SET Header='{$ImageName}' Where UserUID=?
			");
			$query=Database::connect()->prepare($sql);
			$args = array($_SESSION["uuid"]);
			$query->execute($args);
...
';*/
	...
	# Start Template
		Template::doACP_Head("","",true,"6","Welcome to your settings");

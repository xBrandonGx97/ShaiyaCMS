...
$NewPass = isset($_POST["NewPass"]) ? Data::escData(trim($_POST["NewPass"])) : false;
$confNewPass = isset($_POST["confNewPass"]) ? Data::escData(trim($_POST["confNewPass"])) : false;
$errors = array();
$success = false;
...
if(isset($_POST) && !empty($_POST)){
    ...
    # Check Email
    $sql	=	('
					SELECT UserID FROM '.Database::getTable('WEB_PRESENCE').' WHERE UserID=? AND Email=?
	');
	$stmt = Database::connect()->prepare($sql);
    ...
    # Verify DOB
    $sql	=	('
					SELECT UserID FROM '.Database::getTable('WEB_PRESENCE').' WHERE UserID=? AND DOB=?
	');
	$stmt = Database::connect()->prepare($sql);
    ...
    $sql	=	('
					SELECT UserID FROM '.Database::getTable('WEB_PRESENCE').' WHERE SecQuestion=? AND SecAnswer=? AND UserID=?
	');
	$stmt = Database::connect()->prepare($sql);
    ...
    if(count($errors) == 0){
        ...
        $sql2 = ("
                    UPDATE ".Database::getTable('WEB_PRESENCE')."
                    SET Pw=?
                    Where UserID=?
        ");
    ...
    Template::_do_pageHead("Account Recovery");
    if(count($errors)){
        echo '<ul id="error">';

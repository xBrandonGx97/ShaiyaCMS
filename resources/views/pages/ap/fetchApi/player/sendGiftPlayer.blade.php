@if (isset($_POST['submit']))
  @if (empty($data['postChecks']['char']))
    {{$data['gift']->setError('You must enter a character name to send the gift to.')}}
  @elseif (empty($data['postChecks']['itemId']))
  ...
  @elseif (count($data['gift']->verifyChar()) < 1)
    {{$data['gift']->setError('Character doesn\'t exist.')}}
  @endif
  @if(count($data['gift']->getErrors())==0)

    {{$data['gift']->setFormComplete()}}
    @include('pages.ap.fetchApi.player.verifySendGiftPlayer')
  @endif
@endif
